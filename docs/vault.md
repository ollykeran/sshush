...
- **`sshush-op`**: wraps every operation above so a failure can name its reason. A failed request answers with protocol-level *success* and a status byte in the body, because the agent protocol discards the body of a failed extension reply. Requests are `[version][op][payload]`, responses `[version][status][data]`; the payload of an op is byte-for-byte what the matching legacy extension would receive. The legacy extensions remain and are unchanged, so an older `sshush` keeps working against a newer daemon and vice versa. See [Architecture](architecture.md#why-a-failure-knows-its-own-reason).
